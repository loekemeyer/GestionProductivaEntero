...
function renderTable(arts) {
  ...
  if (!arts.length) {
    resultBody.innerHTML = `<tr><td colspan="4" style="text-align:center;color:#b42318;font-weight:700;">Sin artículos.</td></tr>`;
    return;
  }
  ...
  const rows = arts.map((a, i) => `
    <tr data-idx="${i}">
      ...
      <td><input class="input-qty" type="text" inputmode="numeric" placeholder="0" data-role="carton" data-idx="${i}" /></td>
      <td><input class="input-qty" type="text" inputmode="numeric" placeholder="0" data-role="cajas" data-idx="${i}" /></td>
    </tr>
  `).join("");
...
function updateEnviarState() {
  ...
  const hasData = items.some(it => it.carton > 0 || it.cajas > 0);
...
async function seleccionarProv(prov) {
  ...
  try {
    ...
    if (fetchedArts.length) {
      ...
      setTableMsg("Completá Cartón y/o Cajas para los artículos a enviar.");
    } else {
...
function getItemsFromTable() {
  return fetchedArts.map((a, i) => {
    const cartonInput = resultBody.querySelector(`input[data-role="carton"][data-idx="${i}"]`);
    const cajasInput  = resultBody.querySelector(`input[data-role="cajas"][data-idx="${i}"]`);
    return {
      cod: a.Cod_Art || "",
      desc: a.Descripcion || "",
      carton: parseInt(cartonInput?.value || "0") || 0,
      cajas: parseInt(cajasInput?.value || "0") || 0
    };
  });
}
...
btnEnviar.addEventListener("click", async () => {
  ...
  const items = getItemsFromTable().filter(it => it.carton > 0 || it.cajas > 0);
  ...
  const detalle = items.map(it => `${it.cod} - ${it.desc} → Cartón: ${it.carton}, Cajas: ${it.cajas}`).join("\n");
  ...
  try {
    ...
    const { error } = await sb.from(TABLA_ENVIOS).insert(payload);
    if (error) throw error;
...
function imprimirComprobante({ codigo, proveedor, fecha, items }) {
  const rows = items.map(it => `
    <tr>
      ...
      <td style="text-align:right">${it.carton}</td>
      <td style="text-align:right">${it.cajas}</td>
    </tr>
  `).join("");
  ...
  const html = `<!doctype html><html lang="es"><head><meta charset="utf-8" /><title>Comprobante ${codigo}</title>
    ...
    <h1>Comprobante de Envío - Art. Terminado</h1>
    ...
    <table><thead><tr><th>Cod</th><th>Artículo</th><th>Cartón</th><th>Cajas</th></tr></thead><tbody>${rows}</tbody></table>
