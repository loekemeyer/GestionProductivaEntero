...
//
// Autenticacion: verify_jwt = true NO alcanza. Medido el 15/09: una llamada con la clave
// `sb_publishable_...` -que esta escrita en todas las paginas del sistema y es publica por
// diseno- pasa el gateway y devuelve el reporte nominal entero. verify_jwt solo comprueba
// que la clave sea del proyecto, no QUIEN llama. Por eso ademas se exige el token
// REPORTES_TRIGGER_TOKEN (query `?token=` o body `token`), leido de
// lecturacvs.server_secrets por la RPC public.server_secret, que solo ejecuta service_role.
// Sin ese token la funcion contesta 401 y no genera ni sube nada.
//
// Token de Meta: se resuelve en este orden -> secret WA_TOKEN de la funcion, body.wa_token,
// y si no, lecturacvs.server_secrets (REPORTES_WA_TOKEN). Es el mismo token que usa la
// funcion de Damian; al rotarlo en Meta se actualiza esa unica fila.
//
// El PDF es NOMINAL (nombre y apellido, horas y m3 por persona), asi que se sirve con URL
// firmada de 3 dias, no publica. WhatsApp baja el archivo y lo adjunta al mensaje, con lo
// cual al destinatario no se le vence nada.
//
// Invocacion (siempre con el header Authorization Y el token):
//   { "token": "...", "wa_token": "..." }                      -> hoy, a los numeros de prueba
//   { "token": "...", "fecha": "2026-09-08" }                  -> un dia anterior, a prueba
//   { "token": "...", "test": false }                          -> hoy, a Juan
//   { "token": "...", "solo_pdf": true }                       -> sube el PDF y no manda nada
//   { "token": "...", "solo_pdf": true, "desde": "2026-09-08", "hasta": "2026-09-14" }
//   { "token": "...", "diag": true, "desde": "...", "hasta": "..." }  -> auditoria de m3
//
...
// limpiar-reportes-viejos. Ver la nota del encabezado sobre por que va firmada.
...
// Lee un secreto de lecturacvs.server_secrets. La RPC es SECURITY DEFINER y solo la puede
// ejecutar service_role, asi que ni anon ni la clave publishable llegan a la tabla.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // AUTENTICACION. verify_jwt = true es la primera puerta, pero NO alcanza: la clave
    // `sb_publishable_...` esta en todas las paginas del sistema, es publica por diseno y
    // pasa el gateway. Medido el 15/09: con esa clave sola se obtenia el reporte completo.
    //
    // El intento anterior de comparar un token fallaba porque se comparaba contra
    // SUPABASE_SERVICE_ROLE_KEY o contra SEND_WA_TOKEN, valores que esta funcion no
    // controla. Aca el token es propio: se genera y se guarda en lecturacvs.server_secrets,
    // que solo lee service_role. Es el mismo que usa reporte-diario-rendimiento.
    let tokenRecibido = "";
    ...
    if (tokenRecibido !== tokenEsperado) return responder({ error: "unauthorized" }, 401);
    ...
    // URL FIRMADA, no publica: el PDF lleva nombre y apellido de cada operario. Meta la
    // acepta igual (medido el 15/09 contra la plantilla de reporte-diario-rendimiento) y
    // WhatsApp adjunta el archivo, asi que el vencimiento no afecta al destinatario.
    const { data: urlFirmada, error: errFirma } = await sb.storage.from(BUCKET)
      .createSignedUrl(archivo, URL_FIRMADA_SEG);
    if (errFirma || !urlFirmada?.signedUrl) {
      throw new Error("firmando la URL del PDF: " + (errFirma?.message || "sin signedUrl"));
    }
    const pdfUrl = urlFirmada.signedUrl;
    ...
    if (!waToken) {
      ...
      return responder({ error: "falta el token de Meta (body.wa_token, secret WA_TOKEN o "
                                + "REPORTES_WA_TOKEN en lecturacvs.server_secrets)",
                         pdfUrl }, 500);
    }
